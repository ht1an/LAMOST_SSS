...
finput = './NGC_6819.csv'
fcommon = './common_ngc6819.fits'
fgaia = '../../../../catalogue/gaia/base/base.fits'

# --------------------------------------------------
# read catalog from input catalog 
dt = np.dtype([('OBJID', 'S18'), ('RADEG', float), ('DECDEG', float), ('MAG0', float), ('PRI', int)])
t1 = np.genfromtxt(finput, delimiter=',', dtype=dt, skip_header=1)
#f = open(finput, 'r')
...
#center = np.array([132.5512695, 11.8568001])
center = np.array([295.2842790, 40.3255420])
cen_ra = center[0]
cen_dec = center[1]
...
h3 = fits.open(fcommon)
t3 = h3[1].data

t2_sub = t2[id1]
sid3 = t3['source_id']
sid2 = t2_sub['source_id']
id2 = ~np.in1d(sid2, sid3)
t2_sub = t2_sub[id2]
...
pri2 = np.array([20]*len(t), dtype=int)
